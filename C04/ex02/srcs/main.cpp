...
/*   Updated: 2022/04/26 03:32:33 by gleal            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
	...
	// Some tests from previous exercise + examples of instantiation that will not compile (uncomment to check)

	Animal** animal_list = new Animal*[6];
	for (size_t k = 0; k < 6; k++) {
		std::cout << std::endl;
		if ((k % 2) == 0) {
			// animal_list[k] = new Animal();
	...
	delete [] animal_list;
	// New ex02 tests to show that instantiation is not possible
	// Animal test1;
